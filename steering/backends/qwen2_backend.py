...
def _agreement_rel_layer(
    *,
    ...
    # agree_h = sum_k P_last[h,k] * p_all[k], then length-normalize.
...
class Qwen2SteeringAttention(nn.Module):
    """
    Qwen2/Qwen2.5-specific attention wrapper that applies steering levels.

    Implemented against Transformers 4.38.x `transformers.models.qwen2`.
    """
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            if self.layer_idx is None:
                raise ValueError(
                    "Qwen2Attention requires `layer_idx` to be set when using k/v caching."
                )
