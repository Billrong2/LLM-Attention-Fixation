...
DEFAULT_CACHE_DIR = Path(__file__).resolve().parents[1] / ".cache" / "joern_slice"
IDENT_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")

# A vulnerability-oriented default for common unsafe C/C++ sinks.
DEFAULT_SINK_FILTER = (
...
def _cache_key(
    *,
    ...
    digest.update(b"\0")
    digest.update(str(slice_depth).encode("utf-8"))
    digest.update(b"\0")
    digest.update(str(sink_filter or "").encode("utf-8"))
    digest.update(b"\0")
    digest.update(str(joern_cli_dir.resolve()).encode("utf-8"))
...
def generate_joern_slice_graph(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="joern-slice-") as tmp:
        ...
        if sink_filter:
            slice_cmd += ["--sink-filter", str(sink_filter)]
        if parallelism and int(parallelism) > 1:
            slice_cmd += ["--parallelism", str(int(parallelism))]

        _run_checked(slice_cmd, cwd=tmpdir, timeout_sec=timeout_sec)
        slice_json_path = tmpdir / "slices.json"
        if not slice_json_path.is_file():
