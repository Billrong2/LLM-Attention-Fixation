...
def load_manifest(path: Path) -> Tuple[List[Sample], Dict[str, Any]]:
    ...
    for raw_any in rows:
        ...
        for case_index, case_any in enumerate(concrete_cases, start=1):
            ...
            # A concrete case must replace, rather than merely augment, its
            # parent's input/output aliases. Otherwise a parent inline
            # ``stdin`` wins over a case ``stdin_path`` (and likewise for the
            # oracle) when `_optional_text_from_path` checks the direct value
            # first. Keep the alias groups mutually exclusive after merging.
            input_keys = ("stdin", "stdin_path", "input")
            if any(key in case for key in input_keys):
            ...
            argv_keys = ("argv", "args")
            if any(key in case for key in argv_keys):
            ...
            if "case_spec" in case:
                merged["case_spec"] = case["case_spec"]
            ...
            if any(key in case for key in expected_keys):
            ...
            if "expected_stdout_path" in case:
                merged["expected_stdout_path"] = case["expected_stdout_path"]
