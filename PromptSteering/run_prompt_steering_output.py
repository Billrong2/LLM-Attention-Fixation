...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prompt-only steering baseline for Java output prediction."
    )
    ...
    try:
        for snippet, technique, tier, path in items:
            ...
            while completed < target_runs:
                ...
                run_idx = _next_run_index_for_item(item_root, existing_runs) if args.resume == "on" else completed + 1
