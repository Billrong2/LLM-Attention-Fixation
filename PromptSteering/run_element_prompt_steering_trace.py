...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Artifact-conditioned prompt-only steering baseline for Java execution-trace prediction."
    )
    ...
    try:
        for snippet, technique, tier, path, case_id in planned:
            ...
            instruction, structural_prompt = build_artifact_conditioned_instruction(
                ...
                java_code=code,
                task="trace_prediction",
                target_method_name=target.target_method_name,
